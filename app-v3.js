...
<header class="nexmoe-nav">
	<div class="navSize">
			<a href="/"><img class="avatar" src="https://github.com/aditya-shri/apnidrive/profile_cartoon.png"/></a>
			<div class="nav_menu">
				<ul class="menu_ul">
					<li class="menu_li"><a href="https://adityashri.tk/" target="_blank">My Website</a></li>
					<li class="menu_li"><a href="https://github.com/aditya-shri/" target="_blank">Github</a></li>
				</ul>
...
function title(path) {
  ...
  if (model.is_search_page)
    $('title').html(`${document.siteName} - ${drive_name} - Search for
 ${model.q} the result of`);
  else
...
/**
 * Initiate a POST request for listing
 * @param path Path
...
function list(path) {
  ...
  /**
   * Callback after the listing request successfully returns data
  ...
  function successResultCallback(res, path, prevReqParams) {
    ...
    if (res['nextPageToken'] === null) {
      ...
      // If it is not the last page, append data and bind the scroll event (if not yet bound), update scroll_status
      append_files_to_list(path, res['data']['files']);
      if (window.scroll_status.event_bound !== true) {
        // Bound event, if not bound yet
        $(window).on('scroll', function () {
          ...
          if (scrollTop + windowHeight > scrollHeight - (Os.isMobile ? 130 : 80)) {
            /*
                When the scroll to the bottom event is triggered, if it is already loading at this time, ignore this event;
            ...
            requestListPath(path, {
              ...
              // The password is the same as before. Won't show up authError
              null
            )
          }
        });
    ...
    // After loading is successful and the new data is successfully rendered, release the loading lock so that you can continue to process the "scroll to the bottom" event
    if (window.scroll_status.loading_lock === true) {
      window.scroll_status.loading_lock = false
    }
  }
  ...
  requestListPath(path, {password: password},
    ...
    function (path) {
      ...
      session.Storage.setItem('password' + path, pass);
      if (pass != null && pass != "") {
        list(path);
      } else {
        history.go(-1);
...
// The Files to allowed to be displayed
/**
 * Append the requested data of the new page to the list
 * @param path path
 * @param files Requested result
 */
function append_files_to_list(path, files) {
  ...
  for (i in files) {
    ...
    if (item['mimeType'] == 'application/vnd.google-apps.folder') {
      ...
      if ("|srt|txt|mp4|webm|avi|bmp|jpg|jpeg|gif|m4a|mp3|wav|ogg|mpg|mpeg|mkv|mov|".indexOf(`|${ext}|`) >= 0) {
...
/**
 * Render the search result list. There is a lot of repeated code, but there is a different logic in it, so let’s separate it like this for now
 */
function render_search_result_list() {
  ...
  /**
   * Modify the callback after the search request successfully returns data
   * @param res Returned result(object)
   * @param path path
   * @param prevReqParams Parameters used in the request
   */
...
function onSearchResultItemClick(a_ele) {
  ...
  $.post(`/${cur}:id2path`, {id: a_ele.id}, function (data) {
    ...
    dialog = mdui.dialog({
      ...
      content: 'o(╯□╰)o may be because this item does not exist on the disk! It may also be because the files in [Shared with Me] were not added to the personal Drive!',
      history: false,
      modal: true,
      closeOnEsc: true,
      buttons: [
...
// File display: Video |mp4|webm|avi|mpg|mpeg|mkv|mov|
...
// Converting size to readable form 
